...
use crate::types::{Bitboard, Square, SquareVec};
...
pub struct Tables {
    ...
    pub ray_to: SquareMap<SquareMap<Bitboard>>, // All the squares between two squares, includes the destination square, empty bb if not on a line
...
impl Tables {
    ...
    fn init_step_attacks(move_vecs: Vec<SquareVec>) -> SquareMap<Bitboard> {
        let mut table = SquareMap::new([bb!(); 64]);

        for (sq, bb) in table.iter_mut() {
            *bb = move_vecs.iter().fold(bb!(), |acc, vec| match sq + *vec {
    ...
    fn gen_bishop_attacks_slow(sq: Square, occ: Bitboard) -> Bitboard {
        ...
        let north_east = (rank + 1..8)
            .zip(file + 1..8)
            .map(Self::TO_SQ)
            .take_until(sq_occ);
        let south_east = (0..rank)
            .rev()
            .zip(file + 1..8)
            .map(Self::TO_SQ)
            .take_until(sq_occ);
        let south_west = (0..rank)
            .rev()
            .zip((0..file).rev())
            .map(Self::TO_SQ)
            .take_until(sq_occ);
        let north_west = (rank + 1..8)
            .zip((0..file).rev())
            .map(Self::TO_SQ)
            .take_until(sq_occ);

        for sq in north_east
            .chain(south_east)
            .chain(south_west)
            .chain(north_west)
        {
    ...
    fn gen_rook_attacks_slow(sq: Square, occ: Bitboard) -> Bitboard {
        ...
        let north = (rank + 1..8)
            ...
            .map(Self::TO_SQ)
            .take_until(sq_occ);
        let east = repeat(rank)
            .zip(file + 1..8)
            .map(Self::TO_SQ)
            .take_until(sq_occ);
        let south = ((0..rank).rev())
            .zip(repeat(file))
            .map(Self::TO_SQ)
            .take_until(sq_occ);
        let west = repeat(rank)
            .zip((0..file).rev())
            .map(Self::TO_SQ)
            .take_until(sq_occ);
    ...
    fn init_bishop_masks() -> SquareMap<Bitboard> {
        ...
        for (sq, bb) in table.iter_mut() {
            *bb = Self::gen_bishop_attacks_slow(sq, bb!())
                - Bitboard::RANKS[0]
                - Bitboard::RANKS[7]
                - Bitboard::FILES[0]
                - Bitboard::FILES[7];
    ...
    fn init_rook_masks() -> SquareMap<Bitboard> {
        ...
        for (sq, bb) in table.iter_mut() {
            *bb = Self::gen_rook_attacks_slow(sq, bb!());
            if sq.rank() != 0 {
                *bb -= Bitboard::RANKS[0];
            }
            if sq.rank() != 7 {
    ...
    fn init_line_through() -> SquareMap<SquareMap<Bitboard>> {
        let mut table = SquareMap::new([SquareMap::new([bb!(); 64]); 64]);
