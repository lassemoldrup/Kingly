...
fn pieces_placed_correctly_in_starting_pos_fen() {
    let position = StandardPosition::from_fen(STARTING_FEN).unwrap();
...
fn castling_rights_parsed_correctly_from_fen() {
    ...
    let position = StandardPosition::from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w Kq - 0 1").unwrap();
    ...
    let position = StandardPosition::from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w Qk - 0 1").unwrap();
    assert!(!position.castling.get(White, KingSide));
    assert!(position.castling.get(White, QueenSide));
    assert!(position.castling.get(Black, KingSide));
    assert!(!position.castling.get(Black, QueenSide));
...
fn en_passant_parsed_correctly_from_fen() {
    ...
    let position = StandardPosition::from_fen("rnbqkbnr/ppp2ppp/4p3/3pP3/8/8/PPPP1PPP/RNBQKBNR w KQkq d6 0 3").unwrap();
...
fn ply_clock_parsed_correctly_from_fen() {
    ...
    let position = StandardPosition::from_fen("rnbqkb1r/pppppppp/8/8/3Nn3/8/PPPPPPPP/RNBQKB1R w KQkq - 4 3").unwrap();
